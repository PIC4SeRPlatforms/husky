...
def generate_launch_description():
    ...
    primary_lidar_ip = EnvironmentVariable(
        "CPR_LASER_HOST", default_value="192.168.131.20"
    )
    primary_lidar_topic = EnvironmentVariable(
        "CPR_LASER_TOPIC", default_value="front/scan"
    )
    primary_lidar_mount = EnvironmentVariable(
        "CPR_LASER_MOUNT", default_value="front_laser"
    )
    ...
    secondary_lidar_enable = EnvironmentVariable(
        "CPR_LASER_SECONDARY", default_value="false"
    )
    secondary_lidar_model = EnvironmentVariable(
        "CPR_LASER_SECONDARY_MODEL", default_value="ust10"
    )
    secondary_lidar_ip = EnvironmentVariable(
        "CPR_LASER_SECONDARY_HOST", default_value="192.168.131.21"
    )
    secondary_lidar_topic = EnvironmentVariable(
        "CPR_LASER_SECONDARY_TOPIC", default_value="rear/scan"
    )
    secondary_lidar_mount = EnvironmentVariable(
        "CPR_LASER_SECONDARY_MOUNT", default_value="rear_laser"
    )
    ...
    primary_lidar_3d_model = EnvironmentVariable(
        "CPR_3D_LASER_MODEL", default_value="vlp16"
    )
    primary_lidar_3d_ip = EnvironmentVariable(
        "CPR_3D_LASER_HOST", default_value="192.168.131.20"
    )
    primary_lidar_3d_topic = EnvironmentVariable(
        "CPR_3D_LASER_TOPIC", default_value="mid/points"
    )
    primary_lidar_3d_mount = EnvironmentVariable(
        "CPR_3D_LASER_MOUNT", default_value="mid_velodyne"
    )
    ...
    if (primary_lidar_3d_enable.perform(lc)) == "true":
        if primary_lidar_3d_model.perform(lc) == "vlp16":
            ...
            velodyne_pointcloud_params_file = os.path.join(
                velodyne_pointcloud_dir,
                "config",
            ...
            with open(velodyne_pointcloud_params_file, "r") as f:
                config_velodyne_pointcloud_vlp16_params = yaml.safe_load(f)[
                    "velodyne_convert_node"
                ]["ros__parameters"]
            config_velodyne_pointcloud_vlp16_params["calibration"] = os.path.join(
            ...
            with open(velodyne_pointcloud_params_file, "r") as f:
                config_velodyne_pointcloud_vlp32c_params = yaml.safe_load(f)[
                    "velodyne_convert_node"
                ]["ros__parameters"]
            config_velodyne_pointcloud_vlp32c_params["calibration"] = os.path.join(
                velodyne_pointcloud_dir, "params", "VeloView-VLP-32C.yaml"
            )
    ...
    primary_imu_model = EnvironmentVariable(
        "CPR_IMU_MODEL", default_value="microstrain"
    )
    primary_imu_port = EnvironmentVariable(
        "CPR_IMU_PORT", default_value="/dev/microstrain"
    )
    ...
    primary_realsense_enable = EnvironmentVariable(
        "CPR_REALSENSE", default_value="false"
    )
